...
from tasks.asdiv import ASDiv
from tasks.common import TaskMixture
from tasks.customjson import CustomJSON
from tasks.gsm8k import GSM8K
from tasks.mawps import MAWPS
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="output tag for SFT checkpoints (default: d{depth})",
)
parser.add_argument(
    "--load-optimizer",
    type=int,
...
train_tasks = [
    ...
    *[GSM8K(subset="main", split="train") for _ in range(args.gsm8k_epochs)],  # 8K rows per epoch
    ...
    # Note: ASDiv, SVAMP, and MAWPS is included twice in the mixture to give them more weight,
    #  since they are smaller datasets
    *[ASDiv(split="train") for _ in range(4)],  # 2.4K rows of arithmetic word problems with calculator traces
    *[SVAMP(split="train") for _ in range(4)],  # 2.0K rows of arithmetic word problems with calculator traces
    *[MAWPS(split="train") for _ in range(4)],  # 1.1K rows of arithmetic word problems with calculator traces
    MetaMathQA(split="train", stop=80_000),  # 100K-row math ablation slice
    # these amount to ~25K rows
    # fmt: on
]
...
while True:
    ...
    if last_step:
        output_dirname = args.output_tag if args.output_tag else f"d{depth}"  # e.g. d12
