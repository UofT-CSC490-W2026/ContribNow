...
- configurable MLP: relu^2 or SwiGLU
...
class SwiGLUMLP(nn.Module):
    def __init__(self, config):
        ...
        # parameter-count-matched hidden dim for comparison with 4n FFN
        hidden = int((8 * n) / 3)

        # optional: round for efficiency
        hidden = (hidden + 63) // 64 * 64

        self.w1 = Linear(n, hidden, bias=False)  # gate branch
        self.w2 = Linear(n, hidden, bias=False)  # value branch
        self.w3 = Linear(hidden, n, bias=False)  # down projection
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        for each block:
            ...
            attn.c_proj:     zeros
            MLP:             c_fc uniform, c_proj zeros
            SwiGLUMLP:       w1/w2 uniform, w3 zeros
        """
        ...
        for block in self.transformer.h:
            ...
            if isinstance(block.mlp, MLP):
                ...
                raise TypeError(f"Unsupported MLP type: {type(block.mlp)}")
