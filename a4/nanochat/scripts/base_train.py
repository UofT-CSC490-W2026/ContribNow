...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help="number of KV heads for GQA (-1 => use n_head, i.e. MHA)",
)
parser.add_argument(
    "--n-kv-head-ratio",
    type=int,
    default=1,
    help="query:kv head ratio for GQA (n_kv_head = n_head / ratio). Ignored when --n-kv-head is set.",
)
...
parser.add_argument(
    ...
    default=False,
    help="use SwiGLU feed-forward blocks instead of the default ReLU^2 MLP",
)
...
def build_model_meta(depth):
    ...
    assert args.n_kv_head_ratio > 0, (
        f"--n-kv-head-ratio must be positive, got {args.n_kv_head_ratio}"
    )
    if args.n_kv_head != -1:
        n_kv_head = args.n_kv_head
    else:
        assert num_heads % args.n_kv_head_ratio == 0, (
            f"n_head ({num_heads}) must be divisible by n_kv_head_ratio ({args.n_kv_head_ratio})"
        )
        n_kv_head = num_heads // args.n_kv_head_ratio
    assert n_kv_head > 0, f"--n-kv-head must be positive or -1, got {args.n_kv_head}"
    assert n_kv_head <= num_heads, (
        f"--n-kv-head must be <= n_head ({num_heads}), got {n_kv_head}"
    )
    assert num_heads % n_kv_head == 0, (
        f"n_head ({num_heads}) must be divisible by n_kv_head ({n_kv_head})"
    )
    config = GPTConfig(
...
output_dirname = (
    args.model_tag
    if args.model_tag
    else f"d{args.depth}-{'swiglu' if args.use_swiglu else 'relu2'}"
)  # e.g. d12-swiglu or d12-relu2
